...
def carregar_dados():
    ...
    try:
        # Recupera os dados
        response = supabase.table("imoveis").select("*").neq("lat", 0).order("created_at", desc=True).limit(1000).execute()
...
with st.sidebar:
    ...
    with st.form("bug_report"):
        desc = st.text_area("Algo não funcionou?")
        if st.form_submit_button("📢 Reportar") and supabase:
            supabase.table("alertas_clientes").insert({"user_id": "BUG", "termo_busca": desc, "ativo": False, "plano": "beta_global"}).execute()
            st.success("Recebido!")
...
<div class="brand-container">
    ...
    <div class="brand-sub">Encontre o seu lugar no mundo.</div>
</div>
""", unsafe_allow_html=True)
...
with c_main:
    ...
    with c_search:
        local_input = st.text_input("Onde você quer viver?", placeholder="Ex: Figueira da Foz, Porto, São Paulo...", label_visibility="hidden")
...
# --- LÓGICA DO MAPA (SEM PULO) ---
# Se não busca nada, centraliza em Portugal (onde tem mais dados por enquanto)
map_center = [39.55, -7.85] 
zoom_start = 7
ponto_referencia = None

if buscar and local_input:
    geolocator = Nominatim(user_agent="lugar_global_fix_v2")
    try:
        # Aumentei o timeout para evitar erro de lentidão
        loc = geolocator.geocode(local_input, timeout=15)
        ...
        if loc:
            ...
            st.warning("Local não encontrado. Tente cidade e país (Ex: Figueira da Foz, Portugal).")
    except:
        st.error("Erro ao buscar local. Tente novamente.")
...
# --- CORREÇÃO DO GPS ---
# auto_start=False: NÃO puxa o usuário sozinho. 
# Só vai se o usuário clicar no botão do mapa.
...
with c_lead_L:
    ...
    st.write("Aproveite o Beta gratuito. Ao lançarmos a versão final, você terá **20% de desconto vitalício**.")
    st.info("Válido para Portugal e Brasil.")

with c_lead_R:
    with st.form("lista_vip_global_fix"):
        ...
        if btn_cupom and email and supabase:
            supabase.table("alertas_clientes").insert({
                ...
                "ativo": True, 
                "plano": "founder_global"
            }).execute()
            st.balloons()
            st.success("Cupom reservado com sucesso!")