...
# CSS Estilo Google (COM CORREÇÃO DE IMAGEM)
st.markdown("""
    <style>
    ...
    .map-img {
        ...
        width: 100% !important;
        object-fit: cover !important; /* Corta para caber, não estica */
        display: block;
        border-bottom: 1px solid #eee;
    }
    
    .map-info {padding: 12px;}
    .map-price {font-size: 18px; font-weight: 700; color: #1a73e8; margin-bottom: 4px;}
    .map-title {font-size: 14px; color: #202124; margin-bottom: 4px; line-height: 1.2;}
...
if not df.empty:
    for _, row in df.iterrows():
        if row['lat'] != 39.5: 
            ...
            titulo_curto = row.get('titulo', 'Imóvel')[:40] + "..." if len(row.get('titulo', '')) > 40 else row.get('titulo', 'Imóvel')
            
            # HTML RIGOROSO (Largura Fixa + Link na Foto)
            html = f"""
            <div class="map-card">
                ...
                <div class="map-info">
                    ...
                    <div class="map-title" title="{row.get('titulo')}">{titulo_curto}</div>
                    <div style="font-size:11px; color:#666; margin-bottom:8px;">📍 {row.get('endereco', '')}</div>
                    <a href="{row.get('link')}" target="_blank" class="btn-maps">Ver Anúncio</a>
                </div>
            </div>
            """
            
            # Popup com max_width forçado
